...
UOdinRoomJoin *UOdinRoomJoin::JoinRoom(UObject *WorldContextObject, UPARAM(ref) UOdinRoom*& room,
                                       ...
                                       FVector2D initial_position, const FOdinRoomJoinError &onError,
...
UOdinRoomAddMedia *UOdinRoomAddMedia::AddMedia(UObject *WorldContextObject, UPARAM(ref) UOdinRoom*& room,
...
{
    // (new FAutoDeleteAsyncTask<AddMediaTask>(this->Room.Get(), this->CaptureMedia.Get(), this->OnResponse,
    ...
    FFunctionGraphTask::CreateAndDispatchWhenReady(
        [=]() {
            ...
            OdinMediaStreamHandle media_handle = CaptureMedia.IsValid()
                                                     ? CaptureMedia->GetMediaHandle()
                                                     : 0;
...
{
    // (new FAutoDeleteAsyncTask<RemoveMediaTask>(this->Room.Get(), this->CaptureMedia.Get(), this->OnResponse,
    //                                            this->OnError, this->OnSuccess))
    //     ->StartBackgroundTask();
...
UOdinRoomUpdatePosition::UpdatePosition(UObject *WorldContextObject, UPARAM(ref) UOdinRoom *&room,
                                        FVector2D position, const FOdinRoomUpdatePositionError &onError,
...
UOdinRoomUpdatePeerUserData *UOdinRoomUpdatePeerUserData::UpdatePeerUserData(
    ...
    const FOdinRoomUpdatePeerUserDataError &onError, const FOdinRoomUpdatePeerUserDataSuccess &onSuccess)
{
...
UOdinRoomUpdateRoomUserData *UOdinRoomUpdateRoomUserData::UpdateRoomUserData(
    UObject *WorldContextObject, UPARAM(ref) UOdinRoom *&room, const TArray<uint8> &data,
    const FOdinRoomUpdateRoomUserDataError &onError, const FOdinRoomUpdateRoomUserDataSuccess &onSuccess)
{
